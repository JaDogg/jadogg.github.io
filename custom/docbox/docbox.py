    ...
    def parse(self) -> List[Token]:
        ...
        for line in self._get_lines():
            ...
            if mode == "html":
                ...
                tokens.append(Token(TokenType.BULLET, TO_MARKDOWN(text), text, level))
            elif stripped_line.startswith("#"):
                level, text = count_and_strip(stripped_line, "#")
                num = self._num_gen.generate(level)
                id_ = self._id_gen.generate(text)
                mk_header = TO_MARKDOWN(text)
                ...
                tokens.append(
                    Token(TokenType.NOTE, TO_MARKDOWN(stripped_line[1:]), stripped_line[1:]))
            elif stripped_line.startswith("!!!"):
                ...
                tokens.append(
                    Token(TokenType.DEFAULT, TO_MARKDOWN(stripped_line), stripped_line))
...
class ToMarkdownConverter(HtmlConverter):
    def __init__(self, files: List[DocBoxFile], target: str, template_cell: str, template_start: str,
                 template_end: str, minifier_command: str, title: str, desc: str, all_headers: bool,
                 include_meta_after_first_header=True):
        super().__init__(files, target, template_cell, template_start, template_end, minifier_command, title, desc,
                         all_headers, include_meta_after_first_header)
    ...
    def convert(self):
        ...
        for section, doc in self._get_sections():
            if doc_file != doc.file_path:
                doc_file = doc.file_path
                possible_first_header = True
            section.data = section.data.rstrip()
            if section.token_type == TokenType.BULLET:
                content.append(("    " * (section.level - 1)) + "* " + section.data)
            elif section.token_type == TokenType.NOTE or section.token_type == TokenType.NOTE_RAW_HTML:
                note.append(section.data)
            elif section.token_type == TokenType.RAW_HTML:
                content.append(section.data)
            elif section.token_type == TokenType.HEADER:
                content.append("\n\n" + ("#" * section.level)  + " " + section.data + "\n")
            elif section.token_type == TokenType.SEPARATOR:
                cells.append(Cell(NEWLINE.join(content), NEWLINE.join(note)))
                content = []
                note = []
                ...
                content.append(section.data)
        self._toc = ""
        self._write_md(cells)

    def _write_md(self, cells: List[Cell]):
        with open(self._target, "w+", encoding="utf-8") as h:
            for cell in self._put_cell_in_template(cells):
                h.write(cell)
...
TARGET_CONVERTER = HtmlConverter
...
class DocBoxApp:
    ...
    def convert_text(self, arguments, single_file_text: str):
        ...
        TARGET_CONVERTER(doc_objects, self._output_file, self._template_cell,
    ...
    def _parse_arguments(self, arguments):
        ...
        parser.add_argument("--md", dest="md", default=False, action="store_true",
                            help="Use .md files instead of .docbox files")
        parser.add_argument("--export", dest="export", default=False, action="store_true", help="Export to markdown")
        if arguments:
    ...
    def _use_args(self, result):
        global TARGET_CONVERTER, TO_MARKDOWN
        ...
        if result.export:
            ...
            TARGET_CONVERTER = ToMarkdownConverter
            self._to_ext = ".md"
    ...
    def _convert_posts(self, reversed_=False, all_headers=False, posts: str = "posts"):
        ...
        for d in doc_objects:
            ...
            if self._use_markdown_files:
                d.read_more = os.path.join(posts, os.path.basename(d.file_path)[5:].replace(".md", self._to_ext))
            else:
                d.read_more = os.path.join(posts, os.path.basename(d.file_path)[5:].replace(".docbox", self._to_ext))
        ...
        for d in doc_objects:
            ...
            self._id_gen.reset()
            self._num_gen.reset()
            TARGET_CONVERTER([d], target, self._template_cell,
                          self._template_main0, self._template_main1,
